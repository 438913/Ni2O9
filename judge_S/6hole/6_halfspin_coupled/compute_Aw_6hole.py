...
#####################################
def compute_Aw_main(ANi, ACu, epCu, epNi, epbilayer, tpd, tpp, tapzd, tapzp, tz_a1a1, tz_b1b1, pds, pdp, pps, ppp, Upp,
                    Uoo, \
    ...
    if Norb == 8 or Norb == 5:
        fname = 'epCu' + str(epCu) + 'epNi' + str(epNi) + '_tpd' + str(tpd) + '_tpp' + str(tpp) \
                ...
                + '_ACu' + str(ACu) + '_B' + str(B) + '_C' + str(C) + '_tz_a1a1' + str(tz_a1a1) + '_tz_b1b1' + str(
            tz_b1b1)
    ...
    if Norb == 5 or Norb == 6 or Norb == 10 or Norb == 11 or Norb == 12:
        Hint_Ni = ham.create_interaction_matrix_ALL_syms(VS, d_Ni_double, double_Ni_part, idx_Ni, hole3456_Ni_part, \
                                                         S_Ni_val, Sz_Ni_val, AorB_Ni_sym, ACu, ANi)
        Hint_Cu = ham.create_interaction_matrix_ALL_syms(VS, d_Cu_double, double_Cu_part, idx_Cu, hole3456_Cu_part, \
                                                         S_Cu_val, Sz_Cu_val, AorB_Cu_sym, ACu, ANi)
        Hint_po = ham.create_interaction_matrix_po(VS, p_double, apz_double, Upp, Uoo)
        ...
        if pam.if_compute_Aw == 1:
            ...
            fig.compute_Aw1(H, VS, w_vals, d9Ld9L_a1L_b1L_state_indices, d9Ld9L_a1L_b1L_state_labels,
                            "Aw_d9Ld9L_a1L_b1L__", fname)
    ...
    for ANi in pam.ANis:
        ...
        for pressure in [29.5]:
            ...
            if pam.basis_change_type == 'd_double':
                U_Ni, S_Ni_val, Sz_Ni_val, AorB_Ni_sym, \
                    = basis.create_singlet_triplet_basis_change_matrix_d_double(VS, d_Ni_double, double_Ni_part, idx_Ni,
                                                                                hole3456_Ni_part)
                U_Cu, S_Cu_val, Sz_Cu_val, AorB_Cu_sym, \
                    = basis.create_singlet_triplet_basis_change_matrix_d_double(VS, d_Cu_double, double_Cu_part, idx_Cu,
                                                                                hole3456_Cu_part)
                U_coupled, coupled_idx, jm_list = basis.create_coupled_representation_matrix(VS)
                                    ...
                                    for tpp in pam.tpps:
                                        for Upp in pam.Upps:
                                            for Uoo in pam.Uoos:
                                                ...
                                                file_path_list = (
                                                './data/dL_weight', './data/energy_spectrum', './data/orb_max_weight', './data/jm')
                                                ...
                                                compute_Aw_main(ANi, ACu, epCu, epNi, epbilayer, tpd, tpp, tapzd,
                                                                ...
                                                                double_Ni_part, hole3456_Ni_part, \

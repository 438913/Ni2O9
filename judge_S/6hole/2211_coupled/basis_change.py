...
def find_singlet_triplet_partner_d_double(VS, d_part, index, h3456_part):
    ...
    if index == 1456:
        ...
        slabel = h3456_part[0:5] + [d_part[0]] + d_part[6:10] + h3456_part[5:10] + [d_part[5]] + d_part[
                                                                                                 1:5] + h3456_part[
                                                                                                        10:20]
    elif index == 2356:
        slabel = [d_part[0]] + d_part[6:10] + h3456_part[0:5] + h3456_part[5:10] + [d_part[5]] + d_part[
                                                                                                 1:5] + h3456_part[
                                                                                                        10:20]
    elif index == 1256:
        slabel = h3456_part[0:5] + h3456_part[5:10] + [d_part[0]] + d_part[6:10] + [d_part[5]] + d_part[
                                                                                                 1:5] + h3456_part[
                                                                                                        10:20]
    elif index == 1236:
        slabel = h3456_part[0:15] + [d_part[0]] + d_part[6:10] + [d_part[5]] + d_part[1:5] + h3456_part[15:20]
    elif index == 1246:
        slabel = h3456_part[0:10] + [d_part[0]] + d_part[6:10] + h3456_part[10:15] + [d_part[5]] + d_part[
                                                                                                   1:5] + h3456_part[
                                                                                                          15:20]
    elif index == 1346:
        slabel = h3456_part[0:5] + [d_part[0]] + d_part[6:10] + h3456_part[5:15] + [d_part[5]] + d_part[
                                                                                                 1:5] + h3456_part[
                                                                                                        15:20]
    elif index == 2346:
        ...
        if s1 == s2:
            ...
            if o1 == o2:
                ...
                # instead of e1e1 and e2e2
                    ...
                    if idx[i] == 3456:
                        slabel = [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos + hole3456_part[i][0:5] + hole3456_part[i][
                                                                                                   5:20]
                    elif idx[i] == 2456:
                        slabel = [s1, 'dyz'] + dpos + hole3456_part[i][0:5] + [s2, 'dyz'] + dpos + hole3456_part[i][
                                                                                                   5:20]
                    elif idx[i] == 1456:
                        slabel = hole3456_part[i][0:5] + [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos + hole3456_part[i][
                                                                                                   5:20]
                    elif idx[i] == 1356:
                        slabel = hole3456_part[i][0:5] + [s1, 'dyz'] + dpos + hole3456_part[i][5:10] + [s2,
                                                                                                        'dyz'] + dpos + \
                                 hole3456_part[i][10:20]
                    elif idx[i] == 2356:
                        slabel = [s1, 'dyz'] + dpos + hole3456_part[i][0:5] + hole3456_part[i][5:10] + [s2,
                                                                                                        'dyz'] + dpos + \
                                 hole3456_part[i][10:20]
                    elif idx[i] == 1256:
                        slabel = hole3456_part[i][0:5] + hole3456_part[i][5:10] + [s1, 'dyz'] + dpos + [s2,
                                                                                                        'dyz'] + dpos + \
                                 hole3456_part[i][10:20]
                    elif idx[i] == 1236:
                        slabel = hole3456_part[i][0:15] + [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos + hole3456_part[i][
                                                                                                    15:20]
                    elif idx[i] == 1246:
                        slabel = hole3456_part[i][0:10] + [s1, 'dyz'] + dpos + hole3456_part[i][10:15] + [s2,
                                                                                                          'dyz'] + dpos + \
                                 hole3456_part[i][15:20]
                    elif idx[i] == 1346:
                        slabel = hole3456_part[i][0:5] + [s1, 'dyz'] + dpos + hole3456_part[i][5:15] + [s2,
                                                                                                        'dyz'] + dpos + \
                                 hole3456_part[i][15:20]
                    elif idx[i] == 2346:
                        slabel = [s1, 'dyz'] + dpos + hole3456_part[i][0:15] + [s2, 'dyz'] + dpos + hole3456_part[i][
                                                                                                    15:20]
                    elif idx[i] == 1234:
                        slabel = hole3456_part[i][0:20] + [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos
                    elif idx[i] == 1235:
                        slabel = hole3456_part[i][0:15] + [s1, 'dyz'] + dpos + hole3456_part[i][15:20] + [s2,
                                                                                                          'dyz'] + dpos
                    elif idx[i] == 1245:
                        slabel = hole3456_part[i][0:10] + [s1, 'dyz'] + dpos + hole3456_part[i][10:20] + [s2,
                                                                                                          'dyz'] + dpos
                    elif idx[i] == 1345:
                        slabel = hole3456_part[i][0:5] + [s1, 'dyz'] + dpos + hole3456_part[i][5:20] + [s2,
                                                                                                        'dyz'] + dpos
                    elif idx[i] == 2345:
...
def find_singlet_triplet_partner(VS, Ni_layer, Cu_layer, NiorCu, i, Ni_i, Cu_i, pz_layer, pz_i):
    ...
    if NiorCu == 'Ni':
        ...
        if Ni_i == [0, 1]:
            ...
            slabel = mix_layer[0:5] + [Ni_layer[5]] + Ni_layer[1:5] + mix_layer[5:15] + [Ni_layer[0]] + Ni_layer[
                                                                                                        6:10] + mix_layer[
                                                                                                                15:20]
        elif Ni_i == [2, 4]:
            slabel = mix_layer[0:10] + [Ni_layer[5]] + Ni_layer[1:5] + mix_layer[10:15] + [Ni_layer[0]] + Ni_layer[
                                                                                                          6:10] + mix_layer[
                                                                                                                  15:20]
        elif Ni_i == [3, 4]:
        ...
        if Cu_i == [0, 1]:
            ...
            slabel = mix_layer[0:5] + [Cu_layer[5]] + Cu_layer[1:5] + mix_layer[5:15] + [Cu_layer[0]] + Cu_layer[
                                                                                                        6:10] + mix_layer[
                                                                                                                15:20]
        elif Cu_i == [2, 4]:
            slabel = mix_layer[0:10] + [Cu_layer[5]] + Cu_layer[1:5] + mix_layer[10:15] + [Cu_layer[0]] + Cu_layer[
                                                                                                          6:10] + mix_layer[
                                                                                                                  15:20]
        elif Cu_i == [3, 4]:
            ...
            if j == i:
                ...
                if s1 == s2:
                    ...
                    if o1 != 'dxz' and o1 != 'dyz':
                        ...
                        # generate paired e2e2 state:
                        if idx[i2] == 3456:
                            slabel = [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos + hole3456_part[i2][0:5] + hole3456_part[
                                                                                                            i2][5:20]
                        elif idx[i2] == 2456:
                            slabel = [s1, 'dyz'] + dpos + hole3456_part[i2][0:5] + [s2, 'dyz'] + dpos + hole3456_part[
                                                                                                            i2][5:20]
                        elif idx[i2] == 1456:
                            slabel = hole3456_part[i2][0:5] + [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos + hole3456_part[
                                                                                                            i2][5:20]
                        elif idx[i2] == 1356:
                            slabel = hole3456_part[i2][0:5] + [s1, 'dyz'] + dpos + hole3456_part[i2][5:10] + [s2,
                                                                                                              'dyz'] + dpos + \
                                     hole3456_part[i2][10:20]
                        elif idx[i2] == 2356:
                            slabel = [s1, 'dyz'] + dpos + hole3456_part[i2][0:5] + hole3456_part[i2][5:10] + [s2,
                                                                                                              'dyz'] + dpos + \
                                     hole3456_part[i2][10:20]
                        elif idx[i2] == 1256:
                            slabel = hole3456_part[i2][0:5] + hole3456_part[i2][5:10] + [s1, 'dyz'] + dpos + [s2,
                                                                                                              'dyz'] + dpos + \
                                     hole3456_part[i2][10:20]
                        elif idx[i2] == 1236:
                            slabel = hole3456_part[i2][0:15] + [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos + hole3456_part[
                                                                                                             i2][15:20]
                        elif idx[i2] == 1246:
                            slabel = hole3456_part[i2][0:10] + [s1, 'dyz'] + dpos + hole3456_part[i2][10:15] + [s2,
                                                                                                                'dyz'] + dpos + \
                                     hole3456_part[i2][15:20]
                        elif idx[i2] == 1346:
                            slabel = hole3456_part[i2][0:5] + [s1, 'dyz'] + dpos + hole3456_part[i2][5:15] + [s2,
                                                                                                              'dyz'] + dpos + \
                                     hole3456_part[i2][15:20]
                        elif idx[i2] == 2346:
                            slabel = [s1, 'dyz'] + dpos + hole3456_part[i2][0:15] + [s2, 'dyz'] + dpos + hole3456_part[
                                                                                                             i2][15:20]
                        elif idx[i2] == 1234:
                            slabel = hole3456_part[i2][0:20] + [s1, 'dyz'] + dpos + [s2, 'dyz'] + dpos
                        elif idx[i2] == 1235:
                            slabel = hole3456_part[i2][0:15] + [s1, 'dyz'] + dpos + hole3456_part[i2][15:20] + [s2,
                                                                                                                'dyz'] + dpos
                        elif idx[i2] == 1245:
                            slabel = hole3456_part[i2][0:10] + [s1, 'dyz'] + dpos + hole3456_part[i2][10:20] + [s2,
                                                                                                                'dyz'] + dpos
                        elif idx[i2] == 1345:
                            slabel = hole3456_part[i2][0:5] + [s1, 'dyz'] + dpos + hole3456_part[i2][5:20] + [s2,
                                                                                                              'dyz'] + dpos
                        elif idx[i2] == 2345:
...
def create_bonding_anti_bonding_basis_change_matrix(VS):
    ...
    for i in range(0, VS.dim):
        ...
        slabel2 = [s1, orb1, x1, y1, 2 - z1, s2, orb2, x2, y2, 2 - z2, s3, orb3, x3, y3, 2 - z3, s4, orb4, x4, y4,
                   2 - z4, s5, orb5, x5, y5, 2 - z5, s6, orb6, x6, y6, 2 - z6]
...
def create_coupled_representation_matrix(VS, S_Ni_val, Sz_Ni_val, S_Cu_val, Sz_Cu_val):
    ...
    for _ in range(2):
        j_list, jm_list, expand_list = coupling_representation(j_list, j2_list, jm_list, j2m2_list,
                                                               expand_list, expand2_list)
    ...
    # (1). 遍历整个态空间(dim维)，找到具有特定位置和轨道的态(需要变换的态)，并存储对应的索引
    dim = VS.dim
    uncoupled_state = []
    uncoupled_idx = []
    data = []; row = []; col = []
    ...
    for i in range(dim):
        slabel = [['d3z2r2', 0, 0, 2], ['dx2y2', 0, 0, 2], ['px', 1, 0, 2], ['px', -1, 0, 0], ['d3z2r2', 0, 0, 0],
                  ['dx2y2', 0, 0, 0]]
        start_state = VS.get_state(VS.lookup_tbl[i])
        if_slabel = True
        ...
        if if_slabel:
            Sz3 = slabel[2][0]
            Sz3 = half if Sz3 == 'up' else -half

            Sz4 = slabel[3][0]
            Sz4 = half if Sz4 == 'up' else -half
            
            uncoupled_idx.append(i)
            uncoupled_state.append((S_Ni_val[i], Sz_Ni_val[i], S_Cu_val[i], Sz_Cu_val[i], Sz3, Sz4))
            print((S_Ni_val, Sz_Ni_val, S_Cu_val, Sz_Cu_val, Sz3, Sz4))
        else:
    ...
    for i1, expand in enumerate(expand_list):
        ...
        for factor, coef in expand.items():
            ...
            row_idx = uncoupled_idx[idx]  # 根据展开式的项因子(j1, m1, j2, m2, m3, m4)，找到非耦合表象下态的索引

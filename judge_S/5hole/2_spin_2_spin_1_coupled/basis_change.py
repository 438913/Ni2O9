...
def create_coupled_representation_matrix(VS, S_Ni_val, Sz_Ni_val, S_Cu_val, Sz_Cu_val):
    ...
    _, jm_list, expand_list = coupling_representation(j1_list, j2_list, j1m1_list, j2m2_list, expand1_list, expand2_list)

    # (3).调整展开式顺序, 顺序以展开式左边(j, m)为基准，依次按照j, m的大小，升序排列
    dim = len(jm_list)
    ...
    for i in range(dim):
        ...
        orb4 = start_state['hole4_orb']
        orb5 = start_state['hole5_orb']
        orb_list = [orb1, orb2, orb3, orb4, orb5]
